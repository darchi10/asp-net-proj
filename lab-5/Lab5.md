...
- [x]  **Implementirati kompletnu API podršku za sve entitete (CRUD, DTO) (2 boda)**
	- [x]  API controlleri moraju podržavati osnovne CRUD operacije za sve entitete gdje poslovna pravila to dopuštaju
		- [x]  `GET` za dohvat svih zapisa, uz opciju pretrage (query i jos ponesto)
		- [x]  `GET` za dohvat jednog zapisa po ID-u
		- [x]  `POST` za kreiranje zapisa
		- [x]  `PUT` za izmjenu zapisa
		- [x]  `DELETE` za brisanje zapisa
	- [x]  API ne smije direktno izlagati nepotrebna interna polja entiteta
		- [x]  Koristiti DTO klase za podatke koji se vraćaju API klijentu
		- [x]  Povezane podatke prikazati kroz ugniježđene DTO klase gdje ima smisla
	- [x]  API + DTOs za Customer, Phone, Technician, Product, SparePart, RepairJob, Order (OrderItems kroz Order)
- [x]  Upload datoteka mora biti vezan uz konkretnog kviza
	- [x]  Upload raditi asinkrono preko Dropzone komponente ili održavane alternative
	- [x]  Datoteke spremiti na disk
	- [x]  U bazu spremiti metapodatke i putanju
	- [x]  Popis datoteka učitati AJAX pozivom
	- [x]  Omogućiti brisanje postojećih datoteka
- [x]  Autentikacija mora biti uključena kroz [ASP.NET](http://ASP.NET) Core Identity
	- [x]  Lokalna registracija i prijava moraju raditi
	- [x]  `AppUser` mora biti proširen traženim poljima
	- [x]  Autorizacija mora ograničavati akcije prema pravilima zadatka
		- [x]  Javne akcije dostupne su anonimnim korisnicima
		- [x]  Create/Edit/Delete dostupni su samo dopuštenim korisnicima
		- [x]  Role `Admin` i barem još jedna rola moraju biti implementirane
- [x]  Google ili Facebook login mora raditi
